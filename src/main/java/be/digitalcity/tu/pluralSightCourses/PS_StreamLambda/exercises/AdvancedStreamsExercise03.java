        ...
        return products.stream()
                .collect(
                        Collectors.groupingBy(
                                p -> {
                                    if(p.getPrice().compareTo(cheapLimit)<0) return PriceRange.CHEAP;
                                    else if (p.getPrice().compareTo(expensiveLimit)>0) return PriceRange.EXPENSIVE;
                                    else return PriceRange.MEDIUM;
                                },
                                HashMap::new,
                                Collectors.toList()
                        )
